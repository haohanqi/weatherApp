...
export const Header = ({ isLoading, setLoading, setError, setData }: headerProps) => {

	const search =  (city:string) =>{
		setLoading(true)
		fetch(`${process.env.REACT_APP_WEATHER_API_ROOT}/search/?query=${city}`).then(
			async response =>{
				if(response.ok){
			...
			(data)=>{
				let requests:[Promise<Response>] = data.map((item: city|null) => fetch(`${process.env.REACT_APP_WEATHER_API_ROOT}/${item?.woeid}`));
